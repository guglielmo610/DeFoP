...
# else:
#     from realtime_ros_wrapper_infogain_ardupilot import RealtimeRosWrapperInfoGainArdupilot
#     from realtime_ros_wrapper_infogain_ardupilot2 import RealtimeRosWrapperInfoGainArdupilot2
...
class ORACLEPlanner:
    def __init__(self, waypoint_path = None):
        ...
        # new trajectory lib (turn in the middle for some action sequences)
        cmd_yaw_relative_lib = np.linspace(-0.45*FOV_rad, 0.45*FOV_rad, NUM_YAW, dtype=np.float32)
        self.cmd_yaw_relative_1 = np.tile(cmd_yaw_relative_lib, NUM_VEL_X * NUM_VEL_Z)
        self.cmd_yaw_relative_1 = np.reshape(self.cmd_yaw_relative_1, (NUM_SEQUENCE_TO_EVALUATE, 1))
        # if self.run_planner:
        #     print('provaaaaaaaaaaaaaaaa: ',self.run_planner,self.robot_pos)
        #     if self.robot_pos >1:
        #         print("provaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        #         for i in range(0, NUM_SEQUENCE_TO_EVALUATE, 2):
        #             if (self.cmd_vel_z_1[i,0] > 0 * np.deg2rad(PLANNING_VERTICAL_FOV/2)):
        #                 self.cmd_vel_z_2[i,0] = 0

        self.cmd_yaw_relative_2 = np.copy(self.cmd_yaw_relative_1) # yaw command for the end of the action sequences
        

        # for i in range(0, NUM_SEQUENCE_TO_EVALUATE, 2):
        #     if (self.cmd_yaw_relative_2[i,0] > 0.2*FOV_rad):
        #         self.cmd_yaw_relative_2[i,0] = -0.5*FOV_rad
        #     elif (self.cmd_yaw_relative_2[i,0] < -0.2*FOV_rad):cmd_yaw_relative_2
        #         self.cmd_yaw_relative_2[i,0] = 0.5*FOV_rad
        ...
        # yaw_val = self.cmd_yaw_relative_1  # shape: (NUM_SEQUENCE_TO_EVALUATE, 1)
        # self.vel_y = 0.5 * np.tanh(yaw_val)
        # self.vel_y=self.vel_y.reshape(-1, 1)
        # self.cmd_vel_x= self.cmd_vel_x.reshape(-1, 1)
        # self.cmd_vel_z_1= self.cmd_vel_z_1.reshape(-1, 1)
        # self.cmd_vel_z_2= self.cmd_vel_z_2.reshape(-1, 1)
        # self.cmd_vel_z_3= self.cmd_vel_z_3.reshape(-1, 1)
        # self.cmd_yaw_relative_1= self.cmd_yaw_relative_1.reshape(-1, 1)

        
        
        # self.action_seq_1 = np.concatenate((self.cmd_vel_x, self.vel_y, self.cmd_vel_z_1, self.cmd_yaw_relative_1), axis=1) # shape (NUM_SEQUENCE_TO_EVALUATE, ACTION_SHAPE_EVALUATE)
        # self.action_seq_1 = np.reshape(self.action_seq_1, (NUM_SEQUENCE_TO_EVALUATE, 1, ACTION_SHAPE_EVALUATE))
        # self.action_seq_2 = np.concatenate((self.cmd_vel_x, self.vel_y, self.cmd_vel_z_2, self.cmd_yaw_relative_1), axis=1) # shape (NUM_SEQUENCE_TO_EVALUATE, ACTION_SHAPE_EVALUATE)
        # self.action_seq_2 = np.reshape(self.action_seq_2, (NUM_SEQUENCE_TO_EVALUATE, 1, ACTION_SHAPE_EVALUATE))
        # self.action_seq_3 = np.concatenate((self.cmd_vel_x, self.vel_y, self.cmd_vel_z_3, self.cmd_yaw_relative_1), axis=1) # shape (NUM_SEQUENCE_TO_EVALUATE, ACTION_SHAPE_EVALUATE)
        # self.action_seq_3 = np.reshape(self.action_seq_3, (NUM_SEQUENCE_TO_EVALUATE, 1, ACTION_SHAPE_EVALUATE))

        yaw_val = self.cmd_yaw_relative_1.reshape(-1, 1)
        vel_y = 1.5 * np.tanh(yaw_val)

        cmd_vel_x = self.cmd_vel_x.reshape(-1, 1)
        cmd_vel_z_1 = self.cmd_vel_z_1.reshape(-1, 1)
        cmd_vel_z_2 = self.cmd_vel_z_2.reshape(-1, 1)
        cmd_vel_z_3 = self.cmd_vel_z_3.reshape(-1, 1)

        # Safe version using only local reshaped vars
        self.action_seq_1 = np.concatenate((cmd_vel_x, vel_y, cmd_vel_z_1, yaw_val), axis=1)
        self.action_seq_2 = np.concatenate((cmd_vel_x, vel_y, cmd_vel_z_2, yaw_val), axis=1)
        self.action_seq_3 = np.concatenate((cmd_vel_x, vel_y, cmd_vel_z_3, yaw_val), axis=1)
        print('self.action_seq_1 shape:', np.shape(self.action_seq_1))
        self.action_seq_1 = self.action_seq_1.reshape((NUM_SEQUENCE_TO_EVALUATE, 1, ACTION_SHAPE_EVALUATE))
        self.action_seq_2 = self.action_seq_2.reshape((NUM_SEQUENCE_TO_EVALUATE, 1, ACTION_SHAPE_EVALUATE))
        self.action_seq_3 = self.action_seq_3.reshape((NUM_SEQUENCE_TO_EVALUATE, 1, ACTION_SHAPE_EVALUATE))
        print('self.action_seq_1 shape:', np.shape(self.action_seq_1))
        ...
        # print("cmd_vel_x shape:", self.cmd_vel_x.shape)
        # print("vel_y shape:", self.vel_y.shape)
        # print("cmd_vel_z_1 shape:", self.cmd_vel_z_1.shape)
        # print("yaw_val shape:", yaw_val.shape)
        ...
        for i in range(NUM_SEQUENCE_TO_EVALUATE):
            ...
            for j in range(ACTION_HORIZON):
                ...
                cmd_vel_y = self.action_seq[i,j,1]
                cmd_vel_z = self.action_seq[i,j,2]
                cmd_relative_yaw = self.action_seq[i,j,3]
    ...
    def deadend_action(self, min_col_score):
        if (self.deadend_step == 0) and (min_col_score < DEADEND_COL_SCORE_THRESHOLD_HIGH):
            ...
            if self.deadend_step == 0:
                ...
                self.env.yaw_in_spot(np.deg2rad(15.0))
                return ActionResult.RUNNING        
        return ActionResult.RUNNING
    
    def out_of_range_action(self):
        self.out_of_range += 1
        if self.out_of_range > 5:
            self.env.stop_robot()
            self.env.yaw_in_spot(self.robot_yaw + self.goal_yaw)
            print(bcolors.FAIL + 'OUT OF RANGE')
            self.out_of_range=0
        return None
            ...
            if self.run_planner:
                if (self.tick == 1):
                    ...
                    if PLANNING_TYPE != 1:
                        ...
                        # print('shape: ',current_di.shape)
                        # print('size: ',current_di.size)
                    ...
                    print('orientation: ',robot_euler_angles)
                    ...
                    yaw_val = self.cmd_yaw_relative_1.reshape(-1, 1)
                    vel_y = 0.2 * np.tanh(yaw_val)
                    # self.action_seq = np.concatenate((self.cmd_vel_x, self.cmd_vel_z, self.cmd_yaw_relative), axis=1) # shape (NUM_SEQUENCE_TO_EVALUATE, ACTION_SHAPE_EVALUATE)
                    # self.action_seq = np.reshape(self.action_seq, (NUM_SEQUENCE_TO_EVALUATE, 1, ACTION_SHAPE_EVALUATE))
                    # self.action_seq = np.repeat(self.action_seq, ACTION_HORIZON, axis=1) # shape (NUM_SEQUENCE_TO_EVALUATE, ACTION_HORIZON, ACTION_SHAPE_EVALUATE)
                    # self.action_seq = np.ascontiguousarray(self.action_seq, dtype=np.float32)
                    self.action_seq_1 = np.concatenate((self.cmd_vel_x, vel_y, self.cmd_vel_z_1, self.cmd_yaw_relative_1), axis=1) # shape (NUM_SEQUENCE_TO_EVALUATE, ACTION_SHAPE_EVALUATE)
                    self.action_seq_1 = np.reshape(self.action_seq_1, (NUM_SEQUENCE_TO_EVALUATE, 1, ACTION_SHAPE_EVALUATE))
                    self.action_seq_2 = np.concatenate((self.cmd_vel_x, vel_y, self.cmd_vel_z_2, self.cmd_yaw_relative_1), axis=1) # shape (NUM_SEQUENCE_TO_EVALUATE, ACTION_SHAPE_EVALUATE)
                    self.action_seq_2 = np.reshape(self.action_seq_2, (NUM_SEQUENCE_TO_EVALUATE, 1, ACTION_SHAPE_EVALUATE))
                    self.action_seq_3 = np.concatenate((self.cmd_vel_x, vel_y, self.cmd_vel_z_3, self.cmd_yaw_relative_1), axis=1) # shape (NUM_SEQUENCE_TO_EVALUATE, ACTION_SHAPE_EVALUATE)
                    self.action_seq_3 = np.reshape(self.action_seq_3, (NUM_SEQUENCE_TO_EVALUATE, 1, ACTION_SHAPE_EVALUATE))
                    ...
                    if (self.tick == 0):
                        ...
                        if follow_goal:
                            ...
                            #print(self.goal_el)
                            #print(self.goal_yaw)
                            # if self.goal_yaw >2.1 or self.goal_yaw<-2.1:
                            #     self.out_of_range_action()
                            yaw_cost = np.abs((self.action_seq[indx,0,3] + self.robot_yaw - self.goal_yaw + np.pi) % (2 * np.pi) - np.pi)

                            current_yaw = self.action_seq[indx, 0, 3]
                            #print(current_yaw)
                            ...
                            #else:
                            #    self.flip_score *= 0.7  # decay (tune this)
                            #    # self.flip_score = 0
                            ...
                            # Bias if too flippy
                            #if self.flip_score > 1.5:  # ← tune this threshold
                            #    print("-------------------bias applied---------------------------------------------------------------------------------------------------") # preferred_sign = np.sign(sum(self.yaw_direction_history))
                            #    yaw_bias_array = 0.2 * (current_signs != self.prev_yaw_sign).astype(float)
                            ...
                            # print('importante: ', self.action_seq[indx2])
                            # print(self.action_seq[indx2].shape)
                            # print(len(indx2))
                            #print(indx2)
                            #print('lenght of action possibilities: ',len(indx2))
                            el_cost = np.abs( (np.arctan2(self.action_seq[indx2, 0, 2], self.action_seq[indx2, 0, 0]) - self.goal_el + np.pi) % (2 * np.pi) - np.pi)
                            indx_best_in_K = np.argmin(el_cost)
                            indx_best = indx2[indx_best_in_K]
                            # el_cost = np.abs( (np.arctan2(self.action_seq[indx, 0, 1], self.action_seq[indx, 0, 0]) - self.goal_el + np.pi) % (2 * np.pi) - np.pi)
                            # indx_best_in_K = np.argmin(el_cost)
                            # indx2 = indx[indx_best_in_K]
                            # yaw_cost = np.abs((self.action_seq[indx2,0,2] + self.robot_yaw - self.goal_yaw + np.pi) % (2 * np.pi) - np.pi)
                            # indx_yaw_best = np.where(yaw_cost == yaw_cost.min())[0]
                            # indx_best = indx2[indx_yaw_best]
                ...
                if (self.tick == 0):
                    #print(indx_best.size)
                    cmd_evaluate = self.action_seq[indx_best]
                    #print(cmd_evaluate)
                    #print(self.action_seq)
                    yaw_ctrl=self.calculate_yaw_ctrl(cmd_evaluate[self.step_cnt, 3])
                    action = [[cmd_evaluate[self.step_cnt, 0], cmd_evaluate[self.step_cnt, 1], cmd_evaluate[self.step_cnt, 2], yaw_ctrl]]
                    # action = [[cmd_evaluate[self.step_cnt, 0], 0.0, cmd_evaluate[self.step_cnt, 1], cmd_evaluate[self.step_cnt, 2]]]
                    print('cmd',cmd_evaluate[self.step_cnt, 2])
                    print('yaw',self.robot_yaw)
                    print('action: ',action)
