...
#seVAE_CPN_TF_CHECKPOINT_PATH = ['models/10/saved-model-350.hdf5', # SKIP_STEP_GENERATE = 5, ACTION_HORIZON = 15, VEL_MAX = 2.0 m/s
#                        'models/10/saved-model-300.hdf5',
#                        'models/10/saved-model-400.hdf5',
#                        'model_weights/VAE_EVO_back_to_origin/net4/saved-model.hdf5',
#                        'model_weights/VAE_EVO_back_to_origin/net5/saved-model.hdf5']
...
#seVAE_CPN_TF_CHECKPOINT_PATH = ['model_weights/VAE_EVO_back_to_origin/net1/saved-model.hdf5', # SKIP_STEP_GENERATE = 5, ACTION_HORIZON = 15, VEL_MAX = 2.0 m/s
#                        'models/4/saved-model-02.hdf5',
#                        'models/4/saved-model-03.hdf5',
#                        'model_weights/VAE_EVO_back_to_origin/net4/saved-model.hdf5',
#                        'model_weights/VAE_EVO_back_to_origin/net5/saved-model.hdf5']

#'models/4/saved-model-100.hdf5'
# folders containing TRT files (need to run optimize scripts to create TRT files)
seVAE_CPN_TRT_CHECKPOINT_PATH = ['model_weights/VAE_EVO_back_to_origin/nett5',
                                ...
                                'model_weights/VAE_EVO_back_to_origin/net5'] #tt5,tt5,tt6
...
ROBOT_DEPTH_TOPIC =  '/d435/depth/image_filled' #'/d435/depth/image_rect_raw'
ROBOT_ODOM_TOPIC = '/vins_estimator/imu_propagate_body' #'/vins_estimator/odometry' #'/camera/odom/sample' # '/vins_estimator/imu_propagate' # 
ROBOT_CMD_TOPIC = '/mavros/setpoint_velocity/cmd_vel_unstamped'
...
PLANNING_VERTICAL_FOV = 50.0 # degrees
...
#5.0 - 4.5
# collision cost
DEADEND_COL_SCORE_THRESHOLD_HIGH =6.5 #7.8 # 6.4 # allow to yaw in spot to find new free direction
DEADEND_COL_SCORE_THRESHOLD_LOW = 5 # 5.6 # exit yawing in dead end mode 
TIME_WEIGHT_FACTOR = 0.04 # 0.0: zero effect
COLLISION_THRESHOLD = 0.35 #0.2 #0.04 # threshold (compared to the safest action sequence) before checking goal_dir

# waypoint condition and what to do after reaching the waypoints
WAYPOINT_FILE = 'waypoints/waypoint_front.txt'
WAYPOINT_DISTANCE_THRESHOLD = 3.0
WAYPOINT_YAW_THRESHOLD = np.deg2rad(45.0)
ALLOW_YAW_AT_WAYPOINT = True

# Uncertainty-aware
N_E = 3 # number of CPNs in the ensemble
USE_UT = True
...
# Depth image's noise
IMAGE_NOISE_FACTOR = 0.000 # 0 - 0.005
...
MIN_CMD_APPEND = np.array([0.0, -VEL_MAX, -VEL_MAX, -np.deg2rad(HORIZONTAL_FOV/2)])
MAX_CMD_APPEND = np.array([VEL_MAX, VEL_MAX, VEL_MAX, np.deg2rad(HORIZONTAL_FOV/2)])
# MAX_CMD_APPEND = np.array([VEL_MAX, 0.0, 0.0, np.deg2rad(HORIZONTAL_FOV/2)])


############### TRAINING AND OPTIMIZING PARAMS ###############
TRAIN_INFOGAIN = False
EVALUATE_MODE = False # only used when RUN_IN_SIM = True
...
NUM_EPISODES_EVALUATE = 20
EPISODE_TIMEOUT = 100 # seconds
MAX_INITIAL_X = -13.0 #2.5 # meters
MAX_INITIAL_Y = 0.0 #9.5 # meters
MAX_INITIAL_Z = 0.5 # meters
MAX_INITIAL_YAW = 0.0 #-180 # degrees
MIN_INITIAL_X = -13.0 #2.5 # meters
MIN_INITIAL_Y = 0.0 #9.5 # meters
MIN_INITIAL_Z = 0.5 # meters
...
EXPERT_FOLDER = '/home/dgguglielmo/agile_oracle_ws/catkin_aa/src/agile_autonomy/data_generation/data/'
