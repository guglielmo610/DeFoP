...
class RealtimeRosWrapperInfoGainArdupilot:
    def __init__(self):
        ...
        self.trajectory_publisher = rospy.Publisher("/trajectory", MarkerArray, queue_size=1)
        ...
        self.latent_subscriber = rospy.Subscriber(ROBOT_LATENT_TOPIC, Float32MultiArray, self.latent_callback, queue_size=1)
    ...
    def visualize_trajectory(self, trajectory_lib, collision_score_combined, collision_score_timestamp=np.array([]), best_indx=None, worst_indx=None, safe_indx=np.array([]), blocked=np.array([])):
        # print("not blocked: ", blocked)
        # print("all_safe: ", safe_indx)
                ...
                if VISUALIZATION_MODE == 0:
                    ...
                    for i in np.array([points.shape[0]-1]):
                        ...
                        if j == best_indx: 
                            ...
                            marker.color.g = 0.0
                            marker.color.b = 1.0
                        elif j in blocked:  # safe ones (black, medium)
                            marker.scale.x = 0.3
                            marker.scale.y = 0.3
                            marker.scale.z = 0.3
                            marker.color.a = 1.0
    ...
    def get_new_obs(self):
        ...
        # filtered_yaw, safe_mask, blocked, margins = self.filter_actions_by_sectors(depth, self.cmd_yaw_relative, self.FOV_rad, 0.1, 0.02, 0.32)
        ...
        if (len(self.latent_list) == DI_QUEUE_LEN) & (len(self.odom_queue) > 0):
            current_latent = self.latent_list[0]
            self.latent_list.pop() # remove the oldest value
            robot_odom = self.odom_queue[0]
            ...
            # current_mask = None
            valid_obs = False        
        return robot_state, current_latent, None, valid_obs
