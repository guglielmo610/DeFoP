...
parser.add_argument("--experiment_name", type=str, default="6")
...
# Model Paths
BASE_PATH = "/home/dgguglielmo//norv_vae_ws/src/planning/sevae/sevae"
EXPERIMENT_NAME = "6"
...
# Data Path
TFRECORD_FOLDER = os.path.join(BASE_PATH, "tfrecords")
...
def rename_state_dict_keys(state_dict):
  """
  This function adds the prefix "module." to the keys in a state dictionary.

  Args:
      state_dict: The state dictionary to modify.

  Returns:
      A new state dictionary with "module." prepended to the keys.
  """
  new_state_dict = {}
  for key, value in state_dict.items():
...
def load_partial_weights(model, pretrained_dict):
  """Loads a subset of weights from a pre-trained dictionary into the model.

  Args:
      model: The target model to load weights into.
      pretrained_dict: A dictionary containing pre-trained weights.

  Returns:
      A dictionary containing the names of layers that weren't loaded due to mismatched keys.
  """
  pretrained_dict=model_dict = rename_state_dict_keys(pretrained_dict)
  model_dict = model.state_dict()
  #model_dict = rename_state_dict_keys(model_dict)
  unloaded_keys = []
  for name, param in model_dict.items():
    if name in pretrained_dict and param.data.shape == pretrained_dict[name].shape:
...
def process_for_training(input_image, semantic_input_image):
    '''
    Function to process the input image for training
    '''
    # input_image = nn.functional.interpolate(input_image, scale_factor=(0.5625, 0.75), mode='bilinear')
    processed_input_image = input_image.clone()
    
    #processed_filled_input_image = filled_input_image.clone()
    #semantic_input_image = nn.functional.interpolate(semantic_input_image, scale_factor=(0.5625, 0.75), mode='nearest')
    ...
    return processed_input_image_with_noise, image_to_reconstruct, processed_input_image, processed_semantic_input_image
...
def train_model(model, train_dataset_loader, test_dataset_loader, epochs, writer, batch_size, loss_fn, optimizer, save_interval=10):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (num_envs, images_per_env, depth_data, semantic_data) in enumerate(train_dataset_loader):
            ...
            noisy_image, depth_data_to_reconstruct, filtered_data, semantic_data = process_for_training(depth_data, semantic_data)
        ...
        for batch_idx, (num_envs, images_per_env, depth_data, semantic_data) in enumerate(test_dataset_loader):
            model.zero_grad()
            optimizer.zero_grad()
            # Process for testing
...
def main():
    ...
    pretrained_weights = torch.load("/home/dgguglielmo/norv_vae_ws/src/planning/sevae/sevae/weights/seVAE_weights/seVAE_evo_LD_128.pth")
    ...
    # if MULTI_GPU:
    #     model.module.load_state_dict(torch.load("/home/dgguglielmo/norv_vae_ws/src/planning/sevae/sevae/weights/seVAE_weights/seVAE_evo_LD_128.pth"))
    # else:
    #     model.load_state_dict(torch.load("/home/dgguglielmo/norv_vae_ws/src/planning/sevae/sevae/weights/seVAE_weights/seVAE_evo_LD_128.pth"))

    unloaded_keys = load_partial_weights(model, pretrained_weights)

    if unloaded_keys:
        print(f"The following layers were not loaded due to mismatched keys or shapes: {unloaded_keys}")

    # Initialize weights for unloaded layers (assuming default PyTorch initialization)
    for name, param in model.named_parameters():
        if name in unloaded_keys and len(param.shape) >= 2:
    ...
    config = {
        ...
        "multi_gpu": MULTI_GPU,
        "use_filtered_data_to_fill_in_undefined_pixels": False,#FILL_UNDEFINED_PIXELS_WITH_FILTERED_DATA,
