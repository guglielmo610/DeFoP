...
if args.sim == "False":
    IS_SIM = False
    IMAGE_TOPIC = "/d435/depth/image_filled" # "/d435/depth/image_rect_raw"
    USE_CV_BRIDGE = False
...
def bias_depth(depth_img, bias=0.2, min_depth=0.1, max_bias_dist=2.0):
    """
    Apply a depth bias to bring obstacles closer, only if they are within max_bias_dist.
    depth_img is in meters.
    """
    biased_depth = depth_img.copy()
    mask = (depth_img < max_bias_dist) & (depth_img > 0)
    biased_depth[mask] = np.maximum(depth_img[mask] - bias, min_depth)
    return biased_depth
...
class VAENavInterface():
    ...
    def image_callback(self, data):
        ...
        if IS_SIM:
            ...
            #cv_image = bias_depth(cv_image, bias=0.2, min_depth=0.1, max_bias_dist=2.0)
        ...
        #torch_img = torch.from_numpy(input_image).float()
        #torch_img = torch_img.unsqueeze(0).unsqueeze(0)
        #rescaled_image = nn.functional.interpolate(torch_img, scale_factor=(0.75, 0.75), mode='nearest').squeeze(0).squeeze(0)
        ...
        if CALCULATE_RECONSTRUCTION == True:
            ...
            msg_filtered = Image()
            msg_filtered.height = 270 #IMAGE_HEIGHT # hardcoded and not data.height
            msg_filtered.width = 480 #IMAGE_WIDTH # hardcoded and not data.width
            msg_filtered.encoding = "8UC1"
            msg_filtered.is_bigendian = 0
            msg_filtered.step = 480 #IMAGE_WIDTH # 1 byte for each pixel
            #msg_filtered.data = np.reshape(img_filtered_uint8, (IMAGE_WIDTH*IMAGE_HEIGHT,)).tolist()
            msg_filtered.data = np.reshape(img_filtered_uint8, (480*270,)).tolist()
