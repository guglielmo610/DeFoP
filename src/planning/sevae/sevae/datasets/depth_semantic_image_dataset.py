...
class DepthSemanticImageDataset(torch.utils.data.IterableDataset):
    ...
    def read_tfrecord(self, serialized_example):
        ...
        example = tf.io.parse_single_example(serialized_example, feature_description_old)
        ...
        return parsed_num_envs, parsed_num_images_per_env, parsed_depth_tensor, parsed_semantic_tensor
...
def collate_batch(batch):
    ...
    semantic_images = torch.Tensor(np.array(batch[0][3])) #rimetti 4 se usi le filtered images
    

    #return num_envs, num_images_per_env, depth_images, filtered_images, semantic_images
    return num_envs, num_images_per_env, depth_images, semantic_images
